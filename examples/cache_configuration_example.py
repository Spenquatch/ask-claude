...
def demo_cache_configurations():
    ...
    config_short = ClaudeCodeConfig(
        cache_responses=True,
        cache_ttl=300.0  # 5 minutes
    )
    wrapper_short = ClaudeCodeWrapper(config_short)
    print(f"   Cache enabled: {config_short.cache_responses}")
    print(f"   Cache TTL: {config_short.cache_ttl} seconds ({config_short.cache_ttl/60:.0f} minutes)\n")
    ...
    print(f"   Cache TTL: {config_medium.cache_ttl} seconds ({config_medium.cache_ttl/60:.0f} minutes)\n")
...
def demo_cache_behavior():
    ...
    print(f"   Cache hits: {metrics1['cache_hits']}, misses: {metrics1['cache_misses']}")
    ...
    print(f"   Cache hits: {metrics2['cache_hits']}, misses: {metrics2['cache_misses']}")
    ...
    print(f"   Cache hits: {metrics3['cache_hits']}, misses: {metrics3['cache_misses']}")
...
def demo_adaptive_caching():
    ...
    class AdaptiveCacheWrapper:
        ...
        def __init__(self):
            ...
            self.wrappers = {
                'definition': ClaudeCodeWrapper(ClaudeCodeConfig(
                    cache_responses=True,
                    cache_ttl=3600.0  # 1 hour for definitions
                )),
                'calculation': ClaudeCodeWrapper(ClaudeCodeConfig(
                    cache_responses=True,
                    cache_ttl=1800.0  # 30 minutes for calculations
                )),
                'realtime': ClaudeCodeWrapper(ClaudeCodeConfig(
                    cache_responses=False  # No cache for real-time
                )),
                'default': ClaudeCodeWrapper(ClaudeCodeConfig(
                    cache_responses=True,
                    cache_ttl=900.0  # 15 minutes default
                ))
            }
        
        def ask(self, query: str, query_type: str = 'default', **kwargs):
